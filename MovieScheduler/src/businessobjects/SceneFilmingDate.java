 ...
 * in
 *
 * @author Iain Workman, Mitchell Corbett
 */
        ...
        implements BusinessObjectListener
{
    ...
    public boolean hasConflict()
    {
        ...
        while (volunteerIterator.hasNext())
        {
            ...
            {
                if (currentInterval.compareTo(this.getSceneShootingInterval()) == 0)
                {
     ...
     */
    public GregorianCalendar getSceneShootingStart()
    {
     ...
     */
    public GregorianCalendar getSceneShootingEnd()
    {
    ...
    /**
     * sets the private member variable isIgnored to true, so that this
     * filmscenedate will not be detected in the schedule as a conflict because
     * the user has chosen to ignore the reason for this conflict.
     */
    ...
    public BaseBusinessObject clone()
    {
        ...
        GregorianCalendar start
                = (GregorianCalendar) this.sceneShootingInterval.getStart().clone();
        GregorianCalendar end
                = (GregorianCalendar) this.sceneShootingInterval.getEnd().clone();
