 ...
 * derived. The base class includes a validation mechanism, which reports, in a
 * standard way, if the business object is in an invalid state, plus mechanisms
 * for obtaining description of the warning messages pertaining to these errors.
 * For information on deriving a class from this one see updateErrors() and
 * setHasChanged() ToDo: - Implementation - Could do with some sort of event
 * mechanism for signaling that the state has changed, and that the error state
 * has changed.
...
public abstract class BaseBusinessObject implements Cloneable
{
     ...
     *
     * @postcond ~ isValid() == true isNew() == true hasChanged() == false
     */
     ...
     *
     * @return The error messages associated with the BusinessObject, one to a
     * line
     */
    ...
    /**
     * Sets the changed state of the object to the provided changedState This
     * should be called in all setters for derived classes. This flag is then
     * used as an indicator that the object needs to be saved. Usage in derived
     * classes: Let's say we have a class Cat, which contains a field name.
     * Whenever the setter for this member variable is called, we will need to
     * call setHasChanged() to ensure that the ui knows that the object is
     * unsaved, and needs to warn the user before they exit a dialog. void
     * setName(string newName) { name = newName; setHasChanged() } Note: Class
     * is marked final because, though it needs to be accessed by derived
     * classes, it should never be overridden. see
     ...
     * be removed from the object, otherwise the error should be ensured to be
     * present. Usage example in derived classes: Let's say we have a class Cat,
     * which contains a field name. Then let's say we wish to ensure that the
     * name is never empty. Then we can add the following to the setter field
     * for name within Cat: void setName(String newName) { name = newName;
     ...
     * been passed Note: Class is marked final because, though it needs to be
     * accessed by derived classes, it should never be overridden. see
     * http://en.wikipedia.org/wiki/Fragile_base_class for justification.
     */
    ...
    /**
     * The basic toString() method must be overwritten. This should give a
     * simple identifier for the object(example:name)
     *
    ...
    /**
     * This performs a deep clone of the object copying not only itself, but all
     * of the objects that it is comprised of.
     *
    ...
    /**
     * Notifies all the listeners that the changes state of the BusinessObject
     * has been changed
     */
